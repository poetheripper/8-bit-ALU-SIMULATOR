...
`include "up_down_counter_3bit.v"
...
module ALU_Top (
    ...
    output wire [7:0] outbus  //iesire date
);
    ...
    wire cnt1_en = C5 | C8 | C14; // activare contor
    wire cnt1_mode = (C5 | C8) ? 1'b1 : 1'b0; // selecteaza modul de numarare in functie de semnalele de control
					      // incrementare pentru C5 sau C8 / decrementare C14
	
    up_down_counter_3bit inst_cnt1 (.clk(clk), .reset_n(reset), .clr(C0), .en(cnt1_en), .mode(cnt1_mode), .count(cnt1));

    // logica pentru incrementare cnt2 la impartire
    wire cnt2_en = C12;
    wire cnt2_mode = 1'b1; // mereu incrementeaza

    up_down_counter_3bit inst_cnt2 (.clk(clk), .reset_n(reset), .clr(C0), .en(cnt2_en), .mode(cnt2_mode), .count(cnt2));
